...
***Assessment question: do the number of rows/records match up to expectations?***
...
***Assessment question: How much of each represented element/field is empty?***
...
***Assessment question: Does the data need to be converted to intelligible formatting?***
...
***Assessment question: Can the data be enriched with new information?***

### Enriching with External Data & APIs

[explanation]

1. First, [download this Refine project](https://github.com/scottythered/OpenRefine-Workshop/blob/master/Auction-Houses.openrefine.tar.gz?raw=true), which contains some of the addresses of auction houses found in our Schoenberg data.
...
Refine's **Templating** function allows you to export your data in a "roll your own" fashion. As Refine's wiki states, "this is useful for formats that we don't support natively yet, or won't support." Currently, Templating is set up to generate a single JSON document of your data by default; with a few changes, we can set up a template to get our enriched Schoenberg data to an XML file. You'll need to fill out these spaces with the below template:
...
```
   <record>
      ...
      <viafID>{{escape(cells["viafID"].value,"xml")}}</viafID>
